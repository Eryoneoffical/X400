...
class ManualProbeHelper:
    ...
    def cmd_ACCEPT(self, gcmd):
        ...
        gcmd.respond_info(f";{pos[:2]};{start_pos[:2]};{pos[2]};{start_pos[2]}")
        if pos[:2] != start_pos[:2] or pos[2] >= start_pos[2]:
            gcmd.respond_info(
                f"Manual probe failed! Use TESTZ commands to position the\n"
                f"nozzle prior to running ACCEPT;{pos[:2]};{start_pos[:2]};{pos[2]};{start_pos[2]}")
            self.finalize(False)
            return
        self.finalize(True)
