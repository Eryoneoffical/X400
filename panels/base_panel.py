...
class BasePanel(ScreenPanel):
    def __init__(self, screen, title):
        ...
        self.heating_time = time.perf_counter()
       # logging.debug("base_panel=== heating_time:%d" % (self.heating_time))
        self.current_extruder = None
        # Action bar buttons
        abscale = self.bts * 1.8
            ...
            for device in devices:
                ...
                if temp is not None and device in self.labels:
                    ...
                    if device.startswith("extruder"):
                        ...
                        #logging.debug("extruder temperature== %f s:%d" % (temp,time.perf_counter()))
                        if temp > 150 and temp <180:
                            self.heating_time = time.perf_counter()
                           # logging.debug("extruder temperature== %f heating_time:%d" % (temp, self.heating_time ))
                        elif temp < 150:
                            self.heating_time = time.perf_counter()
                        if self._screen.screensaver is not None:
                            if self._printer.state not in ["printing", "paused"] and (time.perf_counter()-self.heating_time)>1200: #1200: # tiemout 20 minutes
                                logging.debug("timenow: %d extruder temperature== %f heating_time_start:%d" % (time.perf_counter(),temp, self.heating_time))
                                script = {"script": "M104 S0"}
    ...
    def show_macro_shortcut(self, show=True):
        if show is True and self.buttons_showing['power'] is False:
            self.action_bar.add(self.control['power'])
            if self.buttons_showing['printer_select'] is False:
                self.action_bar.reorder_child(self.control['power'], 2)
            else:
                self.action_bar.reorder_child(self.control['power'], 3)
            self.control['power'].show()
            self.buttons_showing['power'] = True
        elif show is False and self.buttons_showing['power'] is True:
            self.action_bar.remove(self.control['power'])
            self.buttons_showing['power'] = False

    def toggle_macro_shorcut_sensitive(self, value=True):
        self.control['power'].set_sensitive(value)
