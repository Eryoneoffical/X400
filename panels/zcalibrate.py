...
class Panel(ScreenPanel):
    ...
    def home(self):
        return self._screen._ws.send_method(
    ...
    def wait_confirm(self, dialog, response_id, program):
        ...
        if self.method == "probe":
            ...
            self._screen.show_popup_message("Heating",1,30)
            self._screen._ws.klippy.gcode_script("M104 S200")
            self._screen.show_popup_message("Heating", 1, 30)
            self._screen.show_popup_message("Heating", 1, 30)
            self._screen.show_popup_message("Heating", 1, 30)
            self._screen._ws.klippy.gcode_script("TEMPERATURE_WAIT SENSOR=extruder MINIMUM=150")
    ...
    def start_calibration(self, widget, method):
        ...
        label = Gtk.Label(label=_("Make sure nothing is on the build plate!"))
        vbox.add(label)
        scroll.add(vbox)
        self.dialog_wait = self._gtk.Dialog(self._screen, buttons, scroll, self.wait_confirm, "ddd")
