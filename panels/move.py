...
class Panel(ScreenPanel):
    ...
    def move(self, widget, axis, direction):
        ...
        if "z" not in homed_axes:
            self._screen._ws.klippy.gcode_script("SET_KINEMATIC_POSITION Z=0 X=395")
        if float(self.distance) > 1 and float(self.labels['pos_x'].get_text().split(' ')[1]) == 395.0 :
            self.distance='1'
            dist = f"{direction}{self.distance}"
        try:
            if float(self.labels['pos_z'].get_text().split(' ')[1]) + float(dist) < 0:
                self._screen.show_popup_message("We can't move the z bellow 0", 1)
                return
