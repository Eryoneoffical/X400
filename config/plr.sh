...
#echo 'SET_KINEMATIC_POSITION Z='$1
#echo $1
cat ${2} > /tmp/plrtmpA.$$
sed -i 's/;.*//g' /tmp/plrtmpA.$$
#cat /tmp/plrtmpA.$$ | sed -e '1,/Z'${1}'/ d'| sed  '/; Z/d'  |sed -ne 's/.* Z\([^ ]*\)/SET_KINEMATIC_POSITION Z=\1/p' > ${SD_PATH}/plr.gcode
#cat /tmp/plrtmpA.$$ | sed -e '1,/Z'${1}'/ d' | sed  '/; Z/d'| sed -ne '/ Z/,$ p' | grep -m 1 ' Z' | sed -ne 's/.* Z\([^ ]*\)/SET_KINEMATIC_POSITION Z=\1/p' > ${SD_PATH}/plr.gcode
